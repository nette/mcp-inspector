...
test('tryCreate returns null when Router not configured', function () {
	...
	$toolkit = RouterToolkit::tryCreate(wrapAccessor($container));
...
test('getRoutes returns list of routes', function () {
	...
	$toolkit = new RouterToolkit(accessorWithTypes([Router::class => $router]));
...
test('generateUrl returns generated URL', function () {
	...
	// LinkGenerator is final → use duck-typed anonymous class (Container::getByType returns object).
	...
	$toolkit = new RouterToolkit(accessorWithTypes([
		Router::class => $router,
		LinkGenerator::class => $linkGenerator,
	]));
